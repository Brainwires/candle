...
use wgpu_compute_layer::{
    cache::{BindgroupAlignmentLayout, BufferReferenceId},
    OpIsInplaceable, PipelineIndex, PipelineReference, QueueBuffer, ToU32,
};
...
pub use convert::{
    queue_convert, queue_convert_f16_to_f32, queue_convert_f32_to_f16, queue_convert_f32_to_u8,
...
pub use rms_norm::{queue_layer_norm, queue_rms_norm};
pub use rotary_emb::{queue_rotary_emb_c, queue_rotary_emb_i, queue_rotary_emb_thd};
...
fn add_layout<'a>(
    ...
    if is_contiguous {
        ...
        queue
            .get_meta_mut()
            .extend(stride.iter().map(|&x| x as u32));
    }
}
