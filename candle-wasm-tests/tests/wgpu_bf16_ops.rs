...
#[test]
async fn wgpu_bf16_softmax_trivial_one_elem() -> Result<()> {
    ...
    let gpu_out = y
        ...
        .to_vec1_async::<bf16>()
        .await?;
    ewasm_bindgen_test::console_log!("softmax([5]) -> {:?}", gpu_out);
    assert_bf16_close("bf16 softmax(1 elem)", &[1.0], &gpu_out, 1e-2);
    Ok(())
}
#[test]
async fn wgpu_bf16_softmax_two_elem() -> Result<()> {
    let dev = match try_wgpu() {
        Some(d) => d,
        None => return Ok(()),
    };
    ...
    let gpu_out = y
        ...
        .to_vec1_async::<bf16>()
        .await?;
    ewasm_bindgen_test::console_log!("softmax([0,1]) -> {:?}", gpu_out);
    assert_bf16_close("bf16 softmax(2 elem)", &[0.2689414, 0.7310586], &gpu_out, 1e-2);
    Ok(())
}
#[test]
async fn wgpu_bf16_softmax_last_dim() -> Result<()> {
    let dev = match try_wgpu() {
        Some(d) => d,
        None => return Ok(()),
    };
...
#[test]
async fn wgpu_bf16_rms_norm() -> Result<()> {
...
#[test]
async fn wgpu_bf16_unary_silu() -> Result<()> {
    let dev = match try_wgpu() {
        Some(d) => d,
        None => return Ok(()),
    };
...
#[test]
async fn wgpu_bf16_rotary_emb_rope() -> Result<()> {
    let dev = match try_wgpu() {
        Some(d) => d,
        None => return Ok(()),
    };
