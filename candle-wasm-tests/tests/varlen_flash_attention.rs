...
mod tests {
    ...
    fn rmse(a: &Tensor, reference: &Tensor) -> Result<f32> {
        ...
        let mse = mse_tensor.to_scalar_async::<f32>().await?;
    ...
    fn build_reference_bias(
        ...
        let slopes = if let Some(s) = alibi_slopes {
            ...
            if v.len() != num_heads {
                candle::bail!(
                    "alibi_slopes has len {}, expected {}", v.len(), num_heads
                );
            }
            Some(v)
        } else {
    ...
    fn reference_padded_attention(
        ...
        let seqlens_q_vec = seqlens_q.to_vec1_async::<u32>().await?;
        let seqlens_k_vec = seqlens_k.to_vec1_async::<u32>().await?;
        ...
        let ctx = probs.matmul(&v)?.transpose(1, 2)?;
    ...
    fn assert_close(
        ...
        assert!(mae < mae_tol, "{label}: MAE too large: {mae:.6e} > {mae_tol:.6e}");
        assert!(e < rmse_tol, "{label}: RMSE too large: {e:.6e} > {rmse_tol:.6e}");
        Ok(())
    }
    ...
    fn test_prefill_gqa(impl_fn: VarlenImpl, precision: DType) -> Result<()> {
        let device = Device::Cpu;
        let (batch_size, num_heads, num_kv_heads, head_dim, max_seq) = (
            3,
            12,
            4,
            64,
    ...
    fn test_fused_vs_unfused(causal: bool, precision: DType) -> Result<()> {
        ...
        let (q, k, v) = convert_to_precision(&q, &k, &v, precision)?;
        let softmax_scale = 1.0 / (head_dim as f64).sqrt() as f32;
        let out_fused = VarlenImpl::CpuFlash
            .forward(
                &q,
                &k,
                &v,
        ...
        let out_unfused = VarlenImpl::Unfused
            .forward(
                &q,
                &k,
                &v,
